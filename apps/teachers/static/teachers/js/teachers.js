        ...
        <div class="modal-overlay" id="deleteModal">
            <div class="modal">
                <div class="modal-header">
                    ...
                    <h2 class="modal-title">Öğretmeni Silmek İstediğinize Emin misiniz?</h2>
                </div>
                <div class="modal-body">
                    <p class="modal-message">Bu öğretmeni silmek üzeresiniz. Bu işlem geri alınamaz.</p>
                </div>
...
document.addEventListener('DOMContentLoaded', () => {
    document.addEventListener('click', async (e) => {
        ...
        if (deleteBtn) {
            ...
            // Öğretmen adını mesaja ekle
            const messageEl = modal.querySelector('.modal-message');
            messageEl.textContent = `${teacherName} isimli öğretmeni silmek üzeresiniz. Bu işlem geri alınamaz.`;
            ...
            confirmBtn.addEventListener('click', async () => {
                try {
                    const response = await fetch(`/teachers/${teacherId}/delete/`, {
                        method: 'POST',
                        headers: {
                    ...
                    if (response.ok) {
                        ...
                        const teacherCard = deleteBtn.closest('.teacher-card');
                        teacherCard.style.transition = 'all 0.3s ease';
                        teacherCard.style.transform = 'scale(0.9)';
                        teacherCard.style.opacity = '0';
                        
                        setTimeout(() => {
                            teacherCard.remove();
                            // Eğer hiç öğretmen kalmadıysa
                            const remainingCards = document.querySelectorAll('.teacher-card');
